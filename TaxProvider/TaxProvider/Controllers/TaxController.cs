...
namespace TaxProvider.Controllers
{
    public class TaxController : ApiController
    {
        ...
        public double GetTaxRate(string municipalityName, string dateString)
        {
            ...
            catch (Exception ex)
            {
                Logger.Log(string.Format(
                    "An expcetion was received when getting tax data for user. (MunicipalityName = {0}, date={1}). {2} - {3}",
                    municipalityName, dateString, ex.GetType(), ex.Message));
        ...
        public void PostTaxRecord([FromBody] Tax value)
        {
            ...
            {
                Logger.Log(string.Format(
                    "An expcetion was received when inserting tax data for user. (TaxValue = {0}). {1} - {2}",
                    value.ToString(), ex.GetType(), ex.Message));
                var message = new HttpResponseMessage(HttpStatusCode.InternalServerError)
                {
                    Content = new StringContent("An error has occured while processing your request.")
                };
        ...
        public void Put(int id, [FromBody] Tax value)
        {
            ...
            {
                Logger.Log(string.Format(
                    "An expcetion was received when updating tax data for user. (TaxValue = {0}). {1} - {2}",
                    value.ToString(), ex.GetType(), ex.Message));
                var message = new HttpResponseMessage(HttpStatusCode.InternalServerError)
                {
                    Content = new StringContent("An error has occured while processing your request.")
